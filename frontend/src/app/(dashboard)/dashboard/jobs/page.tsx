...
import { Briefcase, Plus, RefreshCw, XCircle, ChevronLeft, ChevronRight, Filter } from 'lucide-react';
import { formatRelative, getStatusColor, formatDuration, cn } from '@/lib/utils';
import { getSocket } from '@/lib/socket';
import { useDashboardStore } from '@/store';

const JOB_STATUSES = ['ALL', 'QUEUED', 'RUNNING', 'COMPLETED', 'FAILED', 'RETRYING', 'DEAD', 'CANCELLED'];
const JOB_TYPES = ['IMMEDIATE', 'DELAYED', 'CRON', 'RECURRING', 'BATCH'];

function StatusBadge({ status }: { status: string }) {
  return (
    <span className={cn('inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium', getStatusColor(status))}>
      {status}
    </span>
  );
}
...
function CreateJobModal({ queueId, onClose, onCreated }: { queueId: string; onClose: () => void; onCreated: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let payload: any = {};
      try { payload = JSON.parse(form.payload); } catch { setError('Invalid JSON payload'); setLoading(false); return; }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              <label className="block text-sm font-medium mb-1.5">Job Name *</label>
              <Input value={form.name} onChange={(e) => setForm(f => ({ ...f, name: e.target.value }))} placeholder="send-welcome-email" required />
            </div>
            <div>
          ...
          {form.type === 'DELAYED' && (
            <div>
              ...
              <Input type="datetime-local" value={form.runAt} onChange={(e) => setForm(f => ({ ...f, runAt: e.target.value }))} />
            </div>
          )}
          ...
          {form.type === 'CRON' && (
            <div>
              ...
              <Input value={form.cronExpression} onChange={(e) => setForm(f => ({ ...f, cronExpression: e.target.value }))} placeholder="0 9 * * *" />
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium mb-1.5">Tags (comma-sep)</label>
              <Input value={form.tags} onChange={(e) => setForm(f => ({ ...f, tags: e.target.value }))} placeholder="email, marketing" />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1.5">Max Retries</label>
              <Input type="number" min={0} max={100} value={form.maxRetries} onChange={(e) => setForm(f => ({ ...f, maxRetries: Number(e.target.value) }))} />
            </div>
          </div>

          <div>
            <label className="block text-sm font-medium mb-1.5">Idempotency Key</label>
            <Input value={form.idempotencyKey} onChange={(e) => setForm(f => ({ ...f, idempotencyKey: e.target.value }))} placeholder="Optional unique key" />
          </div>

          <div className="flex gap-3 pt-2">
            <Button type="button" variant="outline" onClick={onClose} className="flex-1">Cancel</Button>
            <Button type="submit" loading={loading} className="flex-1">Submit Job</Button>
          </div>
        </form>
      </div>
    </div>
    ...
    <div className="flex flex-col">
      ...
      <div className="p-6 space-y-5">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Button size="sm" variant="outline" onClick={invalidate} className="ml-auto">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border/30">
                      ...
                      <th className="text-left py-3 px-4 text-muted-foreground font-medium">Retries</th>
                  ...
                  <tbody>
                    {jobs.map((job: any) => (
                      <tr key={job.id} className="border-b border-border/10 hover:bg-accent/20 transition-colors">
                        <td className="py-3 px-4">
                          <div>
                            <p className="font-medium text-foreground truncate max-w-[200px]">{job.name}</p>
                        ...
                        <td className="py-3 px-4 text-muted-foreground text-xs">{job.type}</td>
                        <td className="py-3 px-4">
                          <span className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded">{job.priority}</span>
                        </td>
                        <td className="py-3 px-4 text-muted-foreground text-xs">
                          {job.retryCount}/{job.maxRetries}
                        </td>
                        <td className="py-3 px-4 text-muted-foreground text-xs">{formatRelative(job.createdAt)}</td>
                        <td className="py-3 px-4">
                          <div className="flex gap-1">
                            ...
                            {['QUEUED', 'SCHEDULED'].includes(job.status) && (
                              <button
                                onClick={() => handleCancel(job.queueId, job.id)}
